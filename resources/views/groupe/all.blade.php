...
@section('content')
    ...
    <div class="container row ml-md-5 align-middle">
        ...
        <div class="col-sm-8 ">
            ...
            <div class="bg-white p-1 col-12 rounded mt-3">
                <h4 class="mt-3 mb-3"><i class="fas fa-users"></i> Liste des Groupes</h4>
                <div class="col-12"><span></span>
                    <ul class="list-group ">
                        @foreach ($groupeAll as $groupeAll)
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                @if ($groupeAll->image !== null)
                                    <img src="{{asset('storage') . '/' . $groupeAll->image }}" class="rounded-circle image"  alt="...">
                                    {{ $groupeAll->nomGroupe }}
                                @else
                                    <img src="{{asset('image/groupe.png') }}" class="rounded-circle image"  alt="...">
                                    {{ $groupeAll->nomGroupe }}
                                @endif
                                <a href="{{ route('groupe.one', $groupeAll->id) }}"><span class="badge badge-primary badge-pill"> {{ $groupeAll->personnels->count() }} Contact(s)</span></a>
                                <a href="{{ route('show', $groupeAll->id) }}"><i class="fas fa-info-circle"></i></a>
                                <a href="#" style="color: rgb(253, 189, 11);" data-toggle="modal" data-target="#staticBackdropgroupe{{ $groupeAll->id }}"><i class="fas fa-user-edit"></i></a>
                                <div class="modal fade" id="staticBackdropgroupe{{ $groupeAll->id }}" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                                    <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                        <h5 class="modal-title" id="staticBackdropLabel">Modifier le groupe</h5>
                                        ...
                                        <div class="modal-body">
                                            <form class="needs-validation"action="{{ route('groupeupdate', $groupeAll->id) }}" method="POST" novalidate enctype="multipart/form-data">
                                                @csrf
                                                <div class="form-row">
                                                <div class="col-md-12 mb-3">
                                                    <label for="validationCustom01">nom du groupe</label>
                                                    <input type="text" class="form-control col-form-label-sm" name="nom" id="validationCustom01" value="{{ $groupeAll->nomGroupe }}" placeholder="Nom du groupe" required>
                                                    <div class="valid-feedback">
                                                    Valide!
                                                    </div>
                                                </div>
                                                <div class="col-md-12 mb-3">
                                                    <label for="validationCustom01">Description du groupe</label>
                                                    <textarea type="text" class="form-control col-form-label-sm" name="description"  id="validationCustom01" placeholder="Décrivez le groupe" required>{{ $groupeAll->description }}</textarea>
                                                    <div class="valid-feedback">
                                                    Valide!
                                                    </div>
                                                </div>
                                ...
                                <a href="#" data-toggle="modal" data-target="#staticBackdropdeletegroupe{{ $groupeAll->id }}"><i class="far fa-trash-alt" style="color: red;"></i></a>
                                <div class="modal fade" id="staticBackdropdeletegroupe{{ $groupeAll->id }}" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                                    <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                        <h5 class="modal-title" id="staticBackdropLabel">Confirmation de la supréssion</h5>
                                        ...
                                        <div class="modal-body">
                                            <p>Cette action néccessite beaucoup votre attention car si cette action est éffectuée vous perdrez tous contacts de ce groupe</p>
                                            <a href="{{ route('destroy', $groupeAll->id) }}" class="btn btn-danger">Suprimer</a>
                                            <a href="#" class="btn btn-primary">Annuler</a>
                                        </div>
                                        <div class="modal-footer">
                                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Fermer</button>
