    ...
    <div class="container row ml-md-5 align-middle">
        ...
        <div class="col-sm-8 ">
            ...
            <div class="bg-white rounded mt-3 p-3 row">
                <h5>Infomations sur le groupe <span class="badge badge-primary badge-pill">{{ $groupeView->nomGroupe }}</span></h5>
                <p>Les informations ci-dessous sont propre au groupe sélectionné, pout toutes modifications la barre lattérale gauche doit être solicitée</p>
                <div class="col-sm-8 shadow p-3">
                    <h6><b>Description du groupe :</b> {{ $groupeView->description }}</h6>
                    <h6><b>Nombre de contacts pour ce groupe  : <span class="badge badge-danger badge-pill">{{ $groupeView->personnels->count() }}</span></b> :
                    <a href="{{ route('groupe.one', $groupeView->id) }}" class="btn btn-primary mt-3">Afficher tous tes contacts </a>
                    </div>
                <div class="col-sm-4 shadow">
                    @if ($groupeView->image !== null)
                        <img src="{{asset('storage') . '/' . $groupeView->image }}" class="rounded-circle imageview ml-4 p-2"  alt="...">
                    @else
                        <img src="{{asset('image/groupe.png') }}" class="rounded-circle imageview ml-4 p-2"  alt="...">
                        <p>Cette image est une image par défaut car ce groupe ne possède pas d'image</p>
                    @endif
                </div>
            </div>
        </div>
