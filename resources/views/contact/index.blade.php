        ...
        <div class="col-sm-8 ">
            ...
            <div class="bg-white rounded mt-3 p-3 row " style="display: none;" >
              <div class="col-sm-4 bg-white rounded shadow">
                ...
                <a href="#" class="btn btn-primary mb-3">Tout afficher</a>
              </div>
              <div class="col-sm-4 bg-white rounded shadow ">
                    <h5 class="mt-3">
              ...
              <div class="col-sm-4 bg-white rounded shadow ">
                ...
                <p class="mt-3">
                    5 favoris enregistrés
                </p>
                <a href="#" class="btn btn-primary mb-3">Tout afficher</a>
              </div>
            </div>
            <div class="bg-white rounded mt-3 p-3 row">
                <h5>Infomations sur le contact <span class="badge badge-primary badge-pill">{{ $viewContact->nomContact }}</span></h5>
                <p>Les informations ci-dessous sont propre au contact sélectionné, pout toutes modifications la barre lattérale gauche doit être sollicitée</p>
                <div class="col-sm-8 shadow p-3">
                    <h6><b>Nom complet</b> : {{ $viewContact->nomContact }} {{ $viewContact->prenom }}</h6>
                    <h6><b>Profession</b> : {{ $viewContact->profession }}</h6>
                    <div class="diverder mt-3 mb-3" style="height: 0.5px; background-color: rgb(231, 230, 230);"></div>
                    <h6><b>Téléphone</b> : {{ $viewContact->phone }}</h6>
                    <h6><b>E-mail</b> : {{ $viewContact->email }}</h6>
                    <div class="diverder mt-3 mb-3" style="height: 0.5px; background-color: rgb(221, 219, 219);"></div>
                    @if ($viewContact->groupe_id !== null)
                        <h6><b>Groupe</b> : {{ $viewContact->groupe->nomGroupe }}</h6>
                    @else
                        <h6><b>Groupe</b> :
                        </h6>
                    @endif
                    <h6><b>Favoris</b> :
                        @if ($viewContact->favoris == 0)
                            <a href="{{ route('edite', $viewContact->id) }}"><i class="fas fa-heart"></i> Non favorisé, Ajouter à la liste des favoris?</a>
                        @else
                            <a href="{{ route('editefavoris', $viewContact->id) }}"><i class="fas fa-heart" style="color: green;"> Favori</i></a>
                        @endif
                    </h6>
                </div>
                <div class="col-sm-4 shadow">
                    @if ($viewContact->photo !== null)
                        <img src="{{asset('storage') . '/' . $viewContact->photo }}" class="rounded-circle mt-5 ml-4 imageview"  alt="...">
                    @else
                        <img src="{{asset('image/images.png') }}" class="rounded-circle imageview mt-5 ml-4"  alt="...">
                        <p>Cette image est une image par défaut car ce contact ne possède pas d'image</p>
                    @endif
